...
public:
    ...
    public:
        ...
        {
            if (index >= size) 
                throw std::out_of_range("Error! Overflow of the queue!");
            ++index;
            auto s_ptr = ptr.lock();
            ptr = s_ptr->next;
            return *this;
...
void Myqueue<T, ALLOCATOR>::Insert(auto iterator, const T& elem) 
{
    ...
    if (iterator.index == size) 
        ...
        throw(std::runtime_error("Error! Going outside the queue!"));
...
{
    if (size == 0) 
        throw std::runtime_error("Error! Removing from an empty queue!");
